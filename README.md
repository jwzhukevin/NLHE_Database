...
- **材料数据浏览与检索（只读）**：支持分页、排序与多条件筛选
- **高级搜索与过滤**：按名称/化学式/属性等条件筛选
- **晶体结构可视化**：基于 Three.js 的交互式 3D 查看器，支持旋转、缩放与超胞生成
- **能带结构分析**：能带数据可视化显示与分析工具
- **文章内容浏览**：查看相关研究文章
- **数据导入（离线/脚本）**：通过 CLI/脚本批量导入 JSON/CSV 数据（网页端不提供写入）
- **用户认证**：支持登录访问用户功能（不区分管理员）
- **安全防护**：IP 封锁保护机制，防止暴力登录尝试
- **文件格式转换**：支持 TXT 转 DAT/DOCX 的工具
...
### 材料管理（只读）
- **浏览材料**：首页提供分页列表，支持排序与过滤
- **材料详情**：查看详细属性、结构与能带数据

（网页端不提供新增/编辑/删除入口；如需批量导入，请使用 CLI/脚本）
...
- **CLI 管理**：只允许创建普通用户，示例：
  ```bash
  flask user-add <email> <username> <password> user
  ```
  历史命令 `flask admin` 已禁用。
